...
setAnswer(randomWord(words));
  ...
  function generateButtons() {
    return "abcdefghijklmnopqrstuvwxyz".split("").map(ltr => (
      <button className={`Snowman-letter-${ltr}`}
        key={ltr}
        value={ltr}
        onClick={handleGuess}
  ...
  return (
    <div className="Snowman">
      ...
      <p className="Snowman-letters">{nWrong < maxWrong ? generateButtons() : `You lose. The answer is ${answer}.`} </p>
      <button className="Snowman-reset-button" onClick={reset}>Restart.</button>
    </div>
  );
}
